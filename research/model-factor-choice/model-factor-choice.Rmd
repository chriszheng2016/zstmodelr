...
# Test factors list
# test_factors <- c("GPM", "ROCE", "PE", "PB", "CUR", "QR")
# test_factors <- params$test_factors
if ((params$test_factors != "") && !is.null(params$test_factors)) {
  test_factors <- stringr::str_split(params$test_factors, pattern = ",")
  test_factors <- stringr::str_trim(unlist(test_factors))
  msg <- sprintf("Conduct factor choice test for %s.", params$test_factors)
  rlang::inform(msg)
} else {
  rlang::abort("Test factors can't be empty or NULL.")
}
...
exit_time <- Sys.time()
time_used <- lubridate::time_length(exit_time - enter_time, "minute")
time_used <- format(time_used, digits = 3, scientific = FALSE, 
                    big.mark = ",", small.mark = ",", small.interval = 3)     
message(paste("time used:", time_used, "mins"))

```

主要评估参数如下：

* **评估因子组:** `r params$factor_group`

* **评估因子:** `r params$test_factors`
...
```{r fetch_factors_data, cache=FALSE, cache.extra = list(params$test_start_date, params$test_end_date, params$test_factors, params$test_stkcds)}
message("fetch_factors_data...")
enter_time <- Sys.time()
...
test_stkcds <- unique(ds_test_stock_info$stkcd)
...
# Notice: only use keep date and stkcd when conduct spreading, since some 
# factors's indcd could be NA which will lead to duplidated records with 
# same date and stkcd after spreading factor_name and factor_value.
if (all(c("factor_code","factor_value") %in% names(fundamental_factors_raw))) {
  fundamental_factors_raw <- fundamental_factors_raw %>%
   dplyr::select(date, stkcd, factor_code, factor_value) %>%
   tidyr::spread(key = "factor_code", value = "factor_value") %>%
   dplyr::arrange(stkcd, date)
}
...
# Notice: 
# 1. add attribute only work for wide format since it need ind_code 
# and ind_value in long format, 
# 2. nned rebuild indcd since indcd was removed in above process
...
# Filter factors only for list stocks with indcd info
fundamental_factors <- fundamental_factors_raw %>%
  dplyr::filter(trdstat == "list", !is.na(indcd) ) %>%
...
# Filter stocks return for target stocks
if (!is.null(test_stkcds)) {
  stocks_return_raw <- stocks_return_raw %>%
    dplyr::filter(stkcd %in% test_stkcds)
}
...
# Get riskfree rate
riskfree_rate <- get_riskfree_rate(stock_db, period = "month")

# Get excess return of stocks
stocks_excess_return <- stocks_excess_return(stocks_return_raw, 
                                             ts_riskfree_rate = riskfree_rate,
                                             period = "month", 
                                             period_date = "end")
# Use excess return as stock return
stocks_return <- stocks_excess_return %>%
  dplyr::select(date, stkcd, return = excess_return)
...
* **盈利能力因子(Operating Profitability Factors)**

    用于衡量公司获取利润的能力。因素值较大的公司，具有较强的竞争能力，可获得较好的较高的利润。

* **财务风险因子(Finacial Risk Factors)**

    用于衡量一个公司因财务问题而破产的风险，包括短期清偿风险和长期杠杆风险风险：
    
  - **短期清偿风险**：
  用于衡量一个公司不同类别的流动资金偿还负债的能力。清偿能力因素值较大的公司具有更好的资金流动性或清偿债务的能力，并且较少受到于由支付债务和其它负债而破产的威胁;
  
  - **长期杠杆风险**：
  用于衡量公司面临不利市场环境可能面临破产的风险，即对应对销售暂时放缓或突遇经济时，杠杆较高的公司能会由于较大的利润波动而面临较高的财务压力，以至于导致破产。
...
```{r preprocess_test_factors, cache=FALSE, autodep= TRUE}

message("preprocess_test_factors...")
enter_time <- Sys.time()
...
# get lag info of factors
ds_lag_factors <- factors_info %>%
  dplyr::filter(factor_code %in% test_factors) %>%
  dplyr::select(factor_lag_month, factor_code) %>%
  dplyr::group_by(factor_lag_month) %>%
  tidyr::nest(.key = factor_codes) %>%
  dplyr::mutate(factor_codes = purrr::map(
    factor_codes, ~unname(unlist(.x))))

# prepare dataset for lagging factors according factors' lag setting
non_factor_fields <- c("date", "stkcd","indcd")
ds_lag_factors <- ds_lag_factors %>%
  dplyr::mutate(ds_fields = purrr::map(
    factor_codes, ~c(non_factor_fields, .x))) %>%
  dplyr::mutate(ds_origin_factors = purrr::map(ds_fields,
    ~(dplyr::select(fundamental_normal_factors_period, .x))
  ))

# lagging factors according to factors' lag setting
ds_lag_factors <- ds_lag_factors %>%
  dplyr::mutate(ds_lag_factors = purrr::map2(
    .x = ds_origin_factors,
    .y = factor_lag_month,
    .f = ~ts_lag(
      ts_dataset = .x,
...
# combine lagged factor datasets into one dataset
fundamental_normal_factors_lag_period <- purrr::reduce(
      ds_lag_factors$ds_lag_factors,
      ~ dplyr::full_join(.x, .y, by = non_factor_fields)
)


# Combine factor and return into test dataset
# Notice: use inner_joing to ensure all filter on factors also
# applying on return dataset.
ds_factor_test <- stocks_return_period %>%
  dplyr::inner_join(fundamental_normal_factors_lag_period, 
                    by = c("date", "stkcd")) %>%
  dplyr::select(date, stkcd, indcd, dplyr::everything()) %>%
  dplyr::arrange(stkcd, date)

# Save test datatsetz for debug
file_factor_test <- sprintf("ds_factor_test_%s(%s)",
                    params$factor_group, as.character(Sys.Date()))   

saveRDS(ds_factor_test,
  file = file.path("temp", paste0(file_factor_test, ".rds"))
)

readr::write_csv(ds_factor_test,
  path = file.path("temp", paste0(file_factor_test, ".csv"))
)
...
```{r summarize_test_factors}
message("summarize_test_factors...")
enter_time <- Sys.time()

# compute stats of factors in deferent periods
stats_fundamental_normal_factors <- fundamental_normal_factors %>%
  tidyr::gather(
...
stats_fundamental_normal_factors %>%
    ggplot(aes(x = factor_name, y = mean)) +
      geom_boxplot() +
...
summary_fundamental_normal_factors <- stats_fundamental_normal_factors %>%
  dplyr::select(-date) %>%
...
* t.test_t: 对因子收益序列均值非零$t$检验的$t$统计量。当绝对值大于2时，判定均值显著不为0；
* t.test_p: 对因子收益序列均值非零$t$检验的$p$概率值。当小于0.05时，判定均值显著不为0
* normal.test_p: 正态分布检验(Shapiro-Wilk)$p$概率值。当小于0.05时，判定因子收益序列非正态分布。 

```{r factor_test_Uniregression_common, cache=FALSE, autodep= TRUE}
message("factor_test_Uniregression_common...")
enter_time <- Sys.time()

# Build test data dataset
ds_test_uniregression <- ds_factor_test %>%
  tidyr::gather(-c(date, stkcd, indcd, return), 
                key = "factor_name", value = "factor_exposure") %>%
...
message("factor_test_Uniregression_overall...")
enter_time <- Sys.time()
...
message("factor_test_Uniregression_industry...")
enter_time <- Sys.time()
...
message("factor_test_IC_common...")
enter_time <- Sys.time()

# Build test dataset for single period test
ds_test_IC <- ds_factor_test %>%
  tidyr::gather(-c(date, stkcd, indcd, return), 
                key = "factor_name", value = "factor_exposure") %>%
  dplyr::select(date, stkcd, indcd, return, factor_name, factor_exposure)
...
message("factor_test_IC_single_period_overall...")
enter_time <- Sys.time()
...
# message("factor_test_IC_multi_period_overall...")
# enter_time <- Sys.time()
...
message("factor_test_IC_single_period_industry...")
enter_time <- Sys.time()
...
# message("factor_test_IC_multi_period_industry...")
# enter_time <- Sys.time()
...
message("factor_test_portfolio_sorts_common...")
enter_time <- Sys.time()

# Build test data dataset
ds_test_sort_portfolios <- ds_factor_test %>%
  tidyr::gather(-c(date, stkcd, indcd, return), 
                key = "factor_name", value = "factor_exposure") %>%
  dplyr::select(date, stkcd, indcd, return, factor_name, factor_exposure)
...
message("factor_test_portfolio_sorts_overall...")
enter_time <- Sys.time()
...
message("factor_test_portfolio_sorts_industry...")
enter_time <- Sys.time()
...
message("evalute_test_result_common...")
enter_time <- Sys.time()
...
message("evaluate_test_result_overall...")
enter_time <- Sys.time()
...
message("potential_factors_overall...")
enter_time <- Sys.time()
...
message("evaluate_test_result_industry...")
enter_time <- Sys.time()
...
message("potential_factors_industry...")
enter_time <- Sys.time()
...
if (nrow(eval_result_industry$potential_result) > 0) {
  knitr::kable(
    eval_result_industry$potential_result,
    format = "pandoc",
    align = "c",
    digits = 3,
...
end_time <- Sys.time()
total_time_used <- lubridate::time_length(end_time - start_time, "minute")
total_time_used <- format(total_time_used, digits = 3, scientific = FALSE, 
                    big.mark = ",", small.mark = ",", small.interval = 3)
message(paste("time used:", total_time_used, "mins"))
