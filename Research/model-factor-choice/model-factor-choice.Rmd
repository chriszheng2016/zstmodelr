...
```{r factor_test_Uniregression, fig.align= "center"}
...
knitr::kable(
  ...
  align = "c",
  digits = 3,
  caption = "Factor Test of Uniregression"
)
...
* **步骤1、计算每期因子与随后实现收益之间的横截面相关性**
  ...
  在指定时期内的每个时点，单个因素与其后k期实现收益的横截面相关性：
...
* **步骤2、对IC序列进行$t$检验**
  ...
  对信息系数(IC)序列进行$t$检验，判断因子的平均IC的显著性, 同时统计正值/负值比例，对观察IC相关性的方向。
...
knitr::kable(
  ...
  align = "c",
  digits = 3,
  caption = "Factor Test of Information Coefficients(IC)"
)
...
knitr::kable(
  ...
  align = "c",
  digits = 3,
  caption = "Factor Test of Sort Portfolios"
)
...
knitr::kable(
  ...
  align = "c",
  digits = 3,
  caption = "Summary of Portfolios Performance"
)
...
汇总各项测试结果，评估因子的统计显著性和方向性：

* **统计显著性**

通过因子测试结果序列进行$t$检验，当概率p值小于0.05时，说时序列均值显著不为0，即因子是统计显著的。
...
# Significancy evaluation
signif_level <- 0.05
eval_signifcancy <- summary_all_test %>%
  dplyr::select(test, factor_name, t.test_p) %>%
  tidyr::spread(key = test, value = t.test_p) %>%
  dplyr::mutate(signifcant = ifelse(all((uniregression_test <= signif_level), 
                                    (IC_test <= signif_level), 
                                    (sort_portfolios_test <= signif_level)), 
                                    TRUE, FALSE)) %>%
  dplyr::select(factor_name, uniregression_test, IC_test, sort_portfolios_test, 
                signifcant)

signicant_factors <- eval_signifcancy %>%
    dplyr::filter(signifcant == TRUE)%>%
    dplyr::select(factor_name) 

signicant_factors <- signicant_factors$factor_name
...
knitr::kable(
  ...
  align = "c",
  digits = 3,
  caption = "Signifancy of Factors"
)

# Direction evaluation for siginficant factors
if (length(signicant_factors)>0) {
  eval_direction <- summary_all_test %>%
  dplyr::filter(factor_name %in% signicant_factors) %>%
  dplyr::select(test, factor_name, avg) %>%
  tidyr::spread(key = test, value = avg) %>%
  dplyr::select(factor_name, uniregression_test, IC_test, sort_portfolios_test)
