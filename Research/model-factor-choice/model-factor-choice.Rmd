...
if (!params$debug_mode) {
  ...
  fundamental_factors <- get_factors(stock_db,
...
```{r factor_test_Uniregression_common, cache=TRUE, autodep= TRUE}
...
```{r factor_test_IC_common, cache=TRUE, autodep= TRUE}
...
model_compute_IC <- function(df, 
  ...
  switch(neutralize,
         ...
         "industry" = {
           ...
           suppressWarnings({
             IC_result <- cor.test(x = df$return, 
...
result_test_IC_multiperiod <- tibble::tibble(k = 1:k_periods, 
...
```{r factor_test_portfolio_sorts_common, cache=TRUE, autodep= TRUE}
# Build test data dataset
...
evalute_test_result <- function(summary_test_result,
  ...
  eval_result_all <- summary_test_result %>%
    ...
    dplyr::mutate(significant = {ifelse(t.test_p <= signif_level, "yes", "no")},
                  direction = {ifelse(avg > 0,"positive", 
                                        ifelse(avg < 0, "negtive", "NA"))},
                  effect = {ifelse(((direction == "positive") && (odds > 1)) 
                                   || ((direction == "negtive") && (odds < 1)),
                                      "strong", "weak")}) %>%
    dplyr::select(factor_name, test, t.test_p, significant, 
  ...
  # Build results 
  eval_result <- list(potential_factors = potential_factors,
