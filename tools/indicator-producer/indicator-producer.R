...
produce_indictors <- function() {
  ...
  for (i in seq_len(NROW(ds_indicator_defs))) {
    ...
    if (!is.null(ind_def_fun)) {
      ...
      msg <- sprintf("Compute indicator: %s(%s) ...\n", indicator_def$ind_code,
      ...
      if (success) {
        msg <- sprintf("Save indicator: %s(%s) in %s successfully.\n",
                       indicator_def$ind_code,
                       indicator_def$ind_name,
      ...
      msg <- sprintf("Fail to compute indicator: %s(%s), because ind_def_fun is NULL.\n",
                     indicator_def$ind_code,
                     indicator_def$ind_name)
      warning(msg)
    }
